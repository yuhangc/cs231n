...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    captions_out = captions_out + [self._null]
    ...
    # (4) temporal affine transformation to compute scores
  ...
  def sample(self, features, max_length=30):
    ...
    # a loop.                                                                 #
    ###########################################################################
    # get initial state from image features
    h0 = np.dot(features, W_proj) + b_proj
    
    T = max_length
    
    # get each word in sequence
    for t in xrange(T):
        if t == 0:
            cap_in = self._start * np.ones((N, 1), dtype=np.int32)
            h_prev = h0
        ...
        # word embedding
        cap_embed, _ = word_embedding_forward(cap_in, W_embed)
        
        # RNN step
        h = None
        if self.cell_type == 'rnn':
            h, _ = rnn_step_forward(np.squeeze(cap_embed), h_prev, Wx, Wh, b)
        else:
            h, c_next, _ = lstm_step_forward(np.squeeze(cap_embed), h_prev, c_prev, Wx, Wh, b)
